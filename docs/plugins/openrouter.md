...
## Bear In Mind (backend) configuration

This project uses **OpenAI-compatible clients** (via LangChain / OpenAI SDK compatibility). To use **OpenRouter** as the provider, set:
...
- `LLM_MODEL` can be any OpenRouter model id (example in OpenRouter docs: `openai/gpt-5.2`).
- These map directly to the OpenAI SDK pattern described in OpenRouter quickstart: `base_url="https://openrouter.ai/api/v1"` and `api_key="<OPENROUTER_API_KEY>"`. See [OpenRouter Quickstart](https://openrouter.ai/docs/quickstart#using-the-openai-sdk).
